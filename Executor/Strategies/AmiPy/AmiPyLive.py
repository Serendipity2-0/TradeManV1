...
signalsdf = pd.DataFrame(
    ...
    dtype=object,
)  # generic data type for initial creation
...
column_types = {
    ...
    "LongSignal": "Int64",
    "ShortSignal": "Int64",
    "LongCoverSignal": "Int64",
    "ShortCoverSignal": "Int64",
}
...
hist_data = {
    token: pd.DataFrame(
        columns=["date", "instrument_token", "open", "high", "low", "close"]
    )
    for token in nifty_token
}
...
def genSignals(resultdf):
    ...
    def longcover(df, i):
        ...
        return (current_minute not in longcover_indices) and (
            ...
            or (df.index[i].time() > sqroff_time)
        )
...
def on_ticks(ws, ticks):
    ...
    for tick in ticks:
        ...
        hist_data[token].at[current_minute, "high"] = max(
            ltp, hist_data[token].at[current_minute, "high"]
        )
        hist_data[token].at[current_minute, "low"] = min(
            ltp, hist_data[token].at[current_minute, "low"]
        )
    ...
    time_diff = (
        (current_time - last_signal_t)
        if last_signal_t is not None
    ...
    if time_diff >= timedelta(minutes=1):
        ...
        new_signal = (
            signalsdf[
                ["LongSignal", "ShortSignal", "LongCoverSignal", "ShortCoverSignal"]
            ]
            .iloc[-1]
            .any()
        )
...
def update_graph_scatter(n):
    ...
    trade_state_text = (
        f"In Trade: {trade_state['in_trade']}\n"
    ...
    fig.add_annotation(
        ...
        bgcolor=(
            "green"
            if trade_state["trade_type"] == "Long"
            else "red" if trade_state["trade_type"] == "Short" else "grey"
        ),

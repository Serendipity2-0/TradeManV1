...
def strategy_VolumeBreakout(
    stockSymbols, period="2d", duration="1d", volume_change_threshold=3
):
    ...
    for symbol in stockSymbols:
        ...
        if stock_data is not None and len(stock_data) >= 2:
            volume_changes = (
                stock_data["Volume"].pct_change(periods=1).iloc[-2:]
...
def strategy_nr4(stock_symbols, period="5d", duration="1d"):
    ...
    for symbol in stock_symbols:
        stock_data = fetcher.get_stock_data(symbol, period, duration)
        if (
            stock_data is not None and len(stock_data) >= 5
        ):  # Ensure at least 5 days of data for calculation
            stock_data["HighLowRange"] = stock_data["High"] - stock_data["Low"]
            min_range = (
                stock_data["HighLowRange"].tail(4).min()
            )  # Calculate the minimum range in the last 4 days
            today_range = (
                stock_data["High"][-1] - stock_data["Low"][-1]
            )  # Today's range
...
def strategy_golden_crossover(stock_symbols, period="1y", duration="1d"):
    ...
    for symbol in stock_symbols:
        stock_data = fetcher.get_stock_data(symbol, period, duration)
        if (
            stock_data is not None and len(stock_data) >= 26
            ...
            if (
                stock_data["EMA5"].iloc[-2] < stock_data["EMA13"].iloc[-2]
                and stock_data["EMA13"].iloc[-2] < stock_data["EMA26"].iloc[-2]
            ):
                if (
                    stock_data["EMA5"].iloc[-1] > stock_data["EMA13"].iloc[-1]
                    and stock_data["EMA13"].iloc[-1] > stock_data["EMA26"].iloc[-1]
                ):
                    # Calculate the ratio of All-Time High to Last Traded Price
                    all_time_high = stock_data["High"].max()
                    last_traded_price = stock_data["Close"].iloc[-1]
...
def strategy_momentum(stock_symbols):
    ...
    for symbol in stock_symbols:
        ...
        if stock_data is not None and not stock_data.empty:
            ...
            rsi_values = TA_indicators.indicator_RSI(
                stock_data, rsi_length_input, rsi_source_input
            )
    ...
    for symbol in stock_symbols:
        try:
            stock_data = fetcher.get_stock_data(
                symbol, period="2y", duration="1d"
            )  # for 15 min duration = 15m period = 50d, for hourly duration = 1h period = 1y
            stock_data_weekly = fetcher.get_stock_data(
                symbol, period="2y", duration="1wk"
            )
        except:
            pass
        if stock_data is not None and not stock_data.empty:
            ...
            bb_window = 20
            stock_data = TA_indicators.indicator_bollinger_bands(stock_data, bb_window)
            stock_data_weekly = TA_indicators.indicator_bollinger_bands(
            ...
            if rsi_values.iloc[-1] < 40 and stock_data["Above_50_EMA"].iloc[-1]:
                if (
                    ...
                    if (
                        stock_data["Lower_band"].iloc[-2]
                        < stock_data["Lower_band"].iloc[-3]
                    ):
                        if (
                            stock_data["Lower_band"].iloc[-1]
                            > stock_data["Lower_band"].iloc[-2]
                        ):
                            all_time_high = stock_data["High"].max()
                            last_traded_price = stock_data["Close"].iloc[-1]
...
def strategy_BollingerBand_Fail(stock_symbols):
    ...
    for symbol in stock_symbols:
        try:
            stock_data_daily = fetcher.get_stock_data(
                symbol, period="2y", duration="1d"
            )
            stock_data_weekly = fetcher.get_stock_data(
                symbol, period="2y", duration="1wk"
            )
            stock_data_monthly = fetcher.get_stock_data(
                symbol, period="2y", duration="1mo"
            )
        except:
            pass
        # Calculate Bollinger Bands
        bb_window = 20
        stock_data_daily = TA_indicators.indicator_bollinger_bands(
            stock_data_daily, bb_window
        )
        stock_data_weekly = TA_indicators.indicator_bollinger_bands(
            stock_data_weekly, bb_window
        )
        stock_data_monthly = TA_indicators.indicator_bollinger_bands(
            stock_data_monthly, bb_window
        )
            ...
            if stock_data_weekly["MA"].iloc[-1] < stock_data_weekly["Close"].iloc[-1]:
                if stock_data_daily["MA"].iloc[-1] > stock_data_daily["Close"].iloc[-1]:
                    if (
                        ...
                        df_selected_stocks = pd.DataFrame(
                            selected_stocks, columns=["Symbol"]
                        )
                        df_selected_stocks.to_csv(
                            "BollingerBand_Fail.csv", mode="w", header=False
                        )
                    ...
                    if (
                        ...
                        if (
                            ...
                            bollinger_close_to_ema = (
                                abs(
                                    stock_data["Lower_band"].iloc[-1]
                                    - stock_data["EMA_50"].iloc[-1]
                                )
                                < 0.05 * stock_data["Close"].iloc[-1]
                            )

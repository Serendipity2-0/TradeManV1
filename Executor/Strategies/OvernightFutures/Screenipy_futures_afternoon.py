...
def get_strikeprc(instrument_token,strategy_index,prediction):
    ...
    return strategy_obj.calculate_current_atm_strike_prc(base_symbol=strategy_index,token=instrument_token,prediction=prediction, strike_prc_multiplier=strike_prc_multiplier)
...
def message_for_orders(prediction,main_trade_symbol,hedge_trade_symbol,weekly_expiry,monthly_expiry):
    strategy_name = strategy_obj.StrategyName
...
def main():
    ...
    extra_info = {'FuturesExchangeToken':futures_exchange_token,'HedgeExchangeToken':hedge_exchange_token,'Prediction':prediction}

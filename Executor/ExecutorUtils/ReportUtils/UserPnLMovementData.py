...
def fetch_user_tables(user_db_conn):
    ...
    Args:
        user_db_conn (sqlite3.Connection): The SQLite database connection.

    Returns:
        list: A list of dictionaries, each containing a strategy name and its corresponding DataFrame.
    """
    user_tables = []
    ...
    try:
        ...
        all_table_names = [
            table_name[0]
            for table_name in user_db_conn.execute(all_tables_query).fetchall()
        ]
...
def main():
    """
    Main function to calculate PnL summaries and handle potential errors.

    Returns:
        DataFrame: DataFrame containing the overall PnL summary for all users.
    """
    try:

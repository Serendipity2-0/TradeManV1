...
def get_firstock_pnl(user):
    ...
    try:
        ...
        unrealized_total_pnl = sum(
            float(position["unrealizedMTOM"]) for position in positions
        )
        realized_total_pnl = sum(
            float(position["RealizedPNL"]) for position in positions
        )
...
def get_margin_utilized(user_credentials):
    limits = thefirstock.firstock_Limits(userId=user_credentials["BrokerUsername"])
    margin_used = limits.get("data", {}).get("marginused", 0)

...
def get_ins_csv() -> pd.DataFrame:
   """
   Get instruments data from Firstock.
  
   Returns:
       pd.DataFrame: Empty DataFrame since we're bypassing Firstock
   """
   logger.debug("Bypassing Firstock instruments fetch")
   return pd.DataFrame()


async def firstock_placeOrder(
   exchange,
   tradingSymbol,
   quantity,
   price,
   ...
   return {"status": "Failed", "data": "User not LoggedIn"}
...
def get_eq_quote(trading_symbol, user_id):
   quote = thefirstock.firstock_getQuoteLTP(
       exchange="NSE",
...
def firstock_todays_tradebook(user_details):
   ...
   try:
       ...
       logger.error(
           f"Error fetching tradebook for user: {user_details['BrokerUsername']}: {e}"
       )
       return None
...
def fetch_firstock_open_orders(user):
   ...
   try:
       ...
       logger.error(
           f"Error fetching open orders for user: {user['Broker']['BrokerUsername']}: {e}"
       )
...
def firstock_modify_orders_for_users(order_details, users_credentials):
   ...
   segement = Instrument().get_exchange_by_exchange_token(
       str(order_details.get("exchange_token"))
   )
   trading_symbol = Instrument().get_full_format_trading_symbol_by_exchange_token(
       str(order_details.get("exchange_token"))
   )
...
def firstock_create_cancel_order(trade, user):
   ...
   try:
       cancelOrder = thefirstock.firstock_cancelOrder(
           userId=user["Broker"]["BrokerUsername"], orderNumber=trade["orderNumber"]
       )
       logger.info(f"firstock order cancelled: {cancelOrder}")
   except Exception as e:
...
def search_instrument(symbol, user_id):
    ...
    Returns:
        tuple: (instrument_token, trading_symbol) if found, (None, None) otherwise
    """
    try:
        ...
        # First, try to find an exact match for NSE equity
        exact_match = next(
            (inst for inst in instruments 
            if inst.get("exchange") == "NSE" 
            and inst.get("tradingSymbol") == symbol + "-EQ"),
            None
        )
        
        if exact_match:
            return exact_match.get("token"), exact_match.get("tradingSymbol")
        
        # If no exact match, find the first NSE equity instrument containing the symbol
        nse_instruments = [
            inst for inst in instruments 
            if inst.get("exchange") == "NSE" and symbol in inst.get("tradingSymbol", "")
        ]
        
        if nse_instruments:
            return nse_instruments[0].get("token"), nse_instruments[0].get("tradingSymbol")

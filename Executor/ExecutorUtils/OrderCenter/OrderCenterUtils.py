...
def calculate_qty_for_strategies(capital, risk, avg_sl_points, lot_size, qty_amplifier=None, strategy_amplifier=None):
    ...
    try:
        ...
        qty_multiplier = 1 + (qty_amplifier / 100) if qty_amplifier is not None else 1  # qty_multiplier is fetched from the marketinfo
...
def place_order_for_strategy(strategy_users, order_details, order_qty_mode: str = None):
    """
    Place orders for a trading strategy for multiple users.

    Args:
        strategy_users (list): A list of users involved in the strategy.
        order_details (list): A list of order details to be placed.
        order_qty_mode (str, optional): The mode for determining order quantity. Defaults to None.

    Returns:
        list: A list of statuses for all placed orders.
    """

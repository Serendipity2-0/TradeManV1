...
CLIENTS_DB = os.getenv("CLIENTS_DB")
...
def update_fields_mongodb(collection, document_id, data, field_key=None):
    ...
    if field_key is None:
        ...
        collection_ref.update_one(
            {"_id": document_id}, {"$set": {field_key: data}}, upsert=True
        )
...
def push_orders_mongodb(collection, document_id, new_order, field_key=None):
    ...
    if field_key is None:
        collection_ref.update_one(
            {"_id": document_id}, {"$push": {"orders": new_order}}, upsert=True
        )
    else:
        collection_ref.update_one(
            {"_id": document_id},
            {"$push": {f"{field_key}.orders": new_order}},
            upsert=True,
        )
...
def get_client_by_tr_no(tr_no):
    ...
    """
    clients = fetch_collection_data_mongodb(
        CLIENTS_DB
    )  # Ensure CLIENTS_DB is defined and points to your clients collection

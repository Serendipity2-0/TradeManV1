    ...
    for internal_name, actual_sheet_name in sheet_mappings.items():
        try:
            ...
            if "exit_time" in temp_df.columns:
                ...
                temp_df["exit_time"] = pd.to_datetime(
                    temp_df["exit_time"], errors="coerce"
                )
...
def create_dtd_dataframe_updated(data_mappings):
    ...
    all_dates = pd.concat(
        [
            df["exit_time"].dt.date
            for df in data_mappings.values()
    ...
    with pd.ExcelWriter(file_name, engine="openpyxl", mode="a") as writer:
        if "DTD" in writer.book.sheetnames:
            ...
            if "Date" in existing_dtd.columns and "Date" in new_dtd_df.columns:
                last_existing_date = pd.to_datetime(existing_dtd["Date"].iloc[-1])
                new_dtd_df = new_dtd_df[
                    pd.to_datetime(new_dtd_df["Date"]) > last_existing_date
                ]

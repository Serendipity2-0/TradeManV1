...
def process_n_log_trade():
    ...
    for user in active_users.values():
        db_path = os.path.join(db_dir, f"{user['Tr_No']}.db")  # Assuming the DB file name is the same as the Excel file name
        ...
        for strategy_name, strategy_details in strategies.items():
            ...
            for order in orders:
                ...
                entry_time = min([o['time_stamp'] for o in orders if trade_id_prefix in o['trade_id'] and 'EN' in o['trade_id']])
                exit_time = max([o['time_stamp'] for o in orders if trade_id_prefix in o['trade_id'] and 'EX' in o['trade_id']])
            ...
            # i want to append this dict to the processed_trades dict with strategy_name as key
            processed_trades[strategy_name] = processed_trade
            # pprint(processed_trades)

...
def process_csv_file(file_path):
    try:
        ...
        if 'posting_date' in df.columns and 'debit' in df.columns and 'credit' in df.columns:
...
def compare_log_ledger(excel_file_path, csv_file_path):
    ...
    merged_df = pd.merge(excel_df, csv_df, left_on=['Date', 'Day'], right_on=['Date', 'Day'], how='outer')
    ...
    final_df = merged_df[['Sl No_x', 'Date', 'Day', 'NetPnL', 'NetPnL_broker', 'DifferencePnL','DiffPercent']]
...
final_df.to_csv(r'C:\Users\user\OneDrive\Desktop\TradeManV1\SampleData\ledger\kite\log_ledger_compare.csv',index=False)

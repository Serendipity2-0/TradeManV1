...
def get_option_tokens(strike_prc, base_symbol):
    ...
    for option in options:
        exchange_token = instrument_obj.get_exchange_token_by_criteria(
            base_symbol, strike_prc, option, today_expiry
        )
...
def HeikinAshi(ma_df):
    ...
    ha_df["ha_close"] = (
        ma_df["MA_open"] + ma_df["MA_high"] + ma_df["MA_high"] + ma_df["MA_close"]
    ) / 4
    ...
    ha_df["ha_high"] = (
        ma_df["MA_high"].combine(ha_df["ha_close"], max).combine(ha_df["ha_open"], max)
    )

    # HaLow = Min(Low, Min(HaClose, HaOpen))
    ha_df["ha_low"] = (
        ma_df["MA_close"].combine(ha_df["ha_close"], min).combine(ha_df["ha_open"], min)
    )
...
def supertrend(ma_df):
    ...
    for i in range(1, len(ma_df)):
        ...
        if flag == 1:
            up[i] = (
                ha_high[i] + ha_low[i]
            ) / 2 + Supertrend_multiplier * average_true_range[i]

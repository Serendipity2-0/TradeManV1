...
class ShortTermStrategy:
    """
    ShortTerm trading strategy implementation.
    ...
    def __init__(self):
        ...
        log_file = f"Data/Logs/shortterm_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        self.logger.add(
            log_file,
        ...
        self.logger.info("Initialized ShortTerm Strategy")


    def get_active_users(self) -> List[Dict]:
        """
        Get active users from MongoDB who have enabled ShortTerm strategy.
        ...
        try:
            ...
            for user in users_data.values():
                shortterm_config = (
                    user.get("Strategies", {}).get("Equity", {}).get("ShortTerm", {})
                )
                if user.get("Active", False) and shortterm_config:
                    self.logger.info(
                        f"Found active user {user.get('Tr_No')} with ShortTerm strategy"
                    )
        ...
        try:
            ...
            for stock in stocks:
                try:
                    ...
                    instrument_token, trading_symbol = firstock_adapter.search_instrument(
                        eq_symbol, user_id
                    )


                    # Check if instrument was found
                    if instrument_token is None or trading_symbol is None:
                        self.logger.error(
                            f"Could not find instrument for symbol: {eq_symbol}"
                        )
                        continue  # Skip this stock and move to next
                    ...
                    self.logger.info(
                        f"Found instrument token {instrument_token} and trading symbol {trading_symbol} for {eq_symbol}"
                    )
                    ...
                    order_details = {
                        ...
                        "setup": f"MidTerm_{stock['Symbol']}",
                        "trading_symbol": trading_symbol,
                        "exchange_token": str(instrument_token),

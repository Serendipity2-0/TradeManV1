...
from Executor.ExecutorUtils.EquityCenter.EquityCenterUtils import (
    calculate_sma, indicator_rsi, read_stock_data_from_db,calculate_ema
)
...
def perform_tfmomentum_strategy():
    ...
    try:
        ...
        stock_db_path = os.getenv('equity_stock_data_db_path')  # Path to stock data database
        financial_db_path = os.getenv('financial_db_path')  # Path to financial data database
        ...
        for stock_code, stock_data in stock_data_dict.items():
            ...
            df = pd.DataFrame({
                ...
                'Gross Profit Growth': [financial_data['Gross Profit Growth'].iloc[-1]],

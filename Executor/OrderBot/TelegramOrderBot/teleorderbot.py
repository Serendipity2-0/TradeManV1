...
)  # Set to DEBUG to capture detailed logs
...
(
    ...
    TRADE_ID_INPUT,
    STRATEGY_SELECTION,
    STRATEGY_QTY_SELECTION,
    ENTRY_EXIT_SELECTION,
    CONFIRMATION,
) = range(17)
...
active_users = fetch_active_users_from_firebase()
active_users_map = {
    str(idx): user["Profile"]["Name"] for idx, user in enumerate(active_users, 1)
}
...
def strategy_selection(update: Update, context: CallbackContext) -> int:
    ...
    if selected_strategy in ["Extra", "Stocks"]:
        ...
        active_users = fetch_active_users_from_firebase()
        user_selection_message = (
            "Please select one or more users by number (e.g., 1,3,5):\n"
        )
...
def user_selection(update: Update, context: CallbackContext) -> int:
    ...
    )  # The option number for "All Users"
    selected_accounts = []
    ...
    for user_input in user_inputs:
        ...
        if user_input == all_users_option:
            ...
            try:
                selected_account = context.user_data.get("filtered_users", [])[
                    int(user_input) - 1
                ]["Profile"]["Name"]
...
def transaction_type(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        "Select a BaseInstrument by number:\n"
        "1. NIFTY\n"
        "2. BANKNIFTY\n"
        "3. FINNIFTY\n"
        "4. Stock"
...
def product_type(update: Update, context: CallbackContext) -> int:
    ...
    logger.info(
        f"Product type selected: {context.user_data['product_type']}"
    )  # Log for debugging
    update.message.reply_text(
        "Select Option Type:\n" "1. CE\n" "2. PE\n" "3. FUT\n" "4. Stock"
    )
...
def qty_risk_selection(update: Update, context: CallbackContext) -> int:
    ...
    if user_input == "3":
        ...
        prompt = (
            ...
            if user_input == "1"
            else "Please enter the risk percentage:"
        )
...
def entry_exit_selection(update: Update, context: CallbackContext) -> int:
    ...
    summary = (
        f"Order Transaction: {context.user_data.get('order_type')}\n"
    ...
    update.message.reply_text(
        f"Thank you! Here's the summary of your selections:\n{summary}"
    )
...
def process_confirmation(update: Update, context: CallbackContext) -> int:
    ...
    if user_input == "1":
        ...
        update.message.reply_text(
            "Order not placed. Please start again with /start if you wish to place an order."
        )
    else:
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            STRATEGY_SELECTION: [
                MessageHandler(Filters.text & ~Filters.command, strategy_selection)
            ],
            USER_SELECTION: [
            ...
            BASE_INSTRUMENT: [
                MessageHandler(Filters.text & ~Filters.command, base_instrument)
            ],
            STOCK_NAME_INPUT: [
                MessageHandler(Filters.text & ~Filters.command, stock_name_input)
            ],
            PRODUCT_TYPE: [
            ...
            STRIKE_PRICE_SELECTION: [
                MessageHandler(Filters.text & ~Filters.command, strike_price_selection)
            ],
            STRIKE_PRICE_INPUT: [
                MessageHandler(Filters.text & ~Filters.command, strike_price_input)
            ],
            EXPIRY_SELECTION: [
                MessageHandler(Filters.text & ~Filters.command, expiry_selection)
            ],
            QTY_RISK_SELECTION: [
                MessageHandler(Filters.text & ~Filters.command, qty_risk_selection)
            ],
            QTY_RISK_INPUT: [
                MessageHandler(Filters.text & ~Filters.command, qty_risk_input)
            ],
            TRADE_ID_INPUT: [
                MessageHandler(Filters.text & ~Filters.command, trade_id_input)
            ],
            ENTRY_EXIT_SELECTION: [
                MessageHandler(Filters.text & ~Filters.command, entry_exit_selection)
            ],
            CONFIRMATION: [

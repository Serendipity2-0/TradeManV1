...
def get_weeks_for_month(month_number, year):
    ...
    last_day_of_month = datetime.date(year, month_number + 1, 1) - datetime.timedelta(
        days=1
    )
...
def select_client():
    ...
    formatted_client_names = [
        client_name.replace("_", " ").title() for client_name in client_names
    ]
...
def show_profile(selected_client, selected_client_name):
    ...
    if profile_picture is not None:
        ...
        if isinstance(Brokers_list_1, list) and len(Brokers_list_1) > 0:
            ...
            st.markdown(table_style, unsafe_allow_html=True)
            st.write(
                broker_1_df.to_html(index=False, escape=False), unsafe_allow_html=True
            )
        ...
        if isinstance(Brokers_list_2, list) and len(Brokers_list_2) > 0:
            ...
            st.markdown(table_style, unsafe_allow_html=True)
            st.write(
                broker_2_df.to_html(index=False, escape=False), unsafe_allow_html=True
            )
        else:
        ...
        if weekly_saturday_capital:
            ...
            weekly_saturday_capital_value = (
                float(weekly_saturday_capital)
                if isinstance(weekly_saturday_capital, (float, int))
                else 0.0
            )
        ...
        formatted_saturday = next_saturday.strftime(
            "%d-%m-%Y"
        ...
        st.write(
            f"Weekly Saturday Capital for {formatted_saturday}: {formatted_capital_value}"
        )
        ...
        if st.session_state.get("edit_mode", False):
            ...
            if new_profile_picture is not None:
                update_profile_picture(selected_client_name, new_profile_picture)
            ...
            if new_weekly_saturday_capital != weekly_saturday_capital_value:
                updated_data["Weekly Saturday Capital"] = (
                    new_weekly_saturday_capital  # Same key as in the database
                )
                weekly_saturday_capital_value = (
                    new_weekly_saturday_capital  # Update the current value
                )
            ...
            if isinstance(Brokers_list_1, list) and len(Brokers_list_1) > 0:
                for i, broker_1 in enumerate(Brokers_list_1):
                    broker_1["broker_name"] = st.multiselect(
                        "Broker Name",
                        ["Zerodha", "AliceBlue"],
                        default=broker_1.get("broker_name", []),
                        key=f"broker_name_1_{i}",
                    )
                    broker_1["user_name"] = st.text_input(
                        "User Name:",
                        key=f"user_name_1_{i}",
                        value=broker_1.get("user_name", ""),
                    )
                    broker_1["password"] = st.text_input(
                        "Password:",
                        key=f"password_1_{i}",
                        value=broker_1.get("password", ""),
                    )
                    ...
                    broker_1["totp_auth"] = st.text_input(
                        "TotpAuth:",
                        key=f"totp_auth_1_{i}",
                        value=broker_1.get("totp_auth", ""),
                    )
                    broker_1["api_code"] = st.text_input(
                        "ApiCode:",
                        key=f"api_code_1{i}",
                        value=broker_1.get("api_code", ""),
                    )
                    broker_1["api_key"] = st.text_input(
                        "ApiKey:",
                        key=f"api_key_1_{i}",
                        value=broker_1.get("api_key", ""),
                    )
                    broker_1["api_secret"] = st.text_input(
                        "ApiSecret:",
                        key=f"api_secret_1_{i}",
                        value=broker_1.get("api_secret", ""),
                    )
                    broker_1["active"] = st.checkbox(
                        "Active:",
                        key=f"active_1_{i}",
                        value=broker_1.get("active", False),
                    )
                    broker_1["capital"] = st.number_input(
                        "Capital:",
                        key=f"capital_1_{i}",
                        value=broker_1.get("capital", ""),
                    )
                    broker_1["risk_profile"] = st.selectbox(
                        "Risk Profile:",
                        risk_profile_options,
                        key=f"risk_profile_1_{i}",
                        index=risk_profile_options.index(
                            broker_1.get("risk_profile", "Low")
            ...
            if isinstance(Brokers_list_2, list) and len(Brokers_list_2) > 0:
                for i, broker_2 in enumerate(Brokers_list_2):
                    broker_2["broker_name"] = st.multiselect(
                        "Broker Name",
                        ["Zerodha", "AliceBlue"],
                        default=broker_2.get("broker_name", []),
                        key=f"broker_name_2_{i}",
                    )
                    broker_2["user_name"] = st.text_input(
                        "User Name:",
                        key=f"user_name_2_{i}",
                        value=broker_2.get("user_name", ""),
                    )
                    broker_2["password"] = st.text_input(
                        "Password:",
                        key=f"password_2_{i}",
                        value=broker_2.get("password", ""),
                    )
                    ...
                    broker_2["totp_auth"] = st.text_input(
                        "TotpAuth:",
                        key=f"totp_auth_2_{i}",
                        value=broker_2.get("totp_auth", ""),
                    )
                    broker_2["api_code"] = st.text_input(
                        "ApiCode:",
                        key=f"api_code_2{i}",
                        value=broker_2.get("api_code", ""),
                    )
                    broker_2["api_key"] = st.text_input(
                        "ApiKey:",
                        key=f"api_key_2_{i}",
                        value=broker_2.get("api_key", ""),
                    )
                    broker_2["api_secret"] = st.text_input(
                        "ApiSecret:",
                        key=f"api_secret_2_{i}",
                        value=broker_2.get("api_secret", ""),
                    )
                    broker_2["active"] = st.checkbox(
                        "Active:",
                        key=f"active_2_{i}",
                        value=broker_2.get("active", False),
                    )
                    broker_2["capital"] = st.number_input(
                        "Capital:",
                        key=f"capital_2_{i}",
                        value=broker_2.get("capital", ""),
                    )
            ...
            if isinstance(Strategy_list, list) and len(Strategy_list) > 0:
                for i, strategy in enumerate(Strategy_list):
                    ...
                    default_strategy_names = [
                        ...
                        if name in strategy_options
                    ]
                    default_brokers = [
                        broker
                        for broker in strategy.get("broker", [])
                        if broker in broker_options
                    ]
                    ...
                    for selected_strategy in selected_strategies:
                        for selected_broker_name in selected_brokers:
                            ...
                            default_index = (
                                options.index(percentage_allocated)
                                if percentage_allocated in options
                                else 0
                            )
            ...
            if st.button("Update"):
                if (
                    updated_data
                    or updated_brokers_list_1
                    or updated_brokers_list_2
                    or updated_strategies_list
                ):
                    if updated_data:
                    ...
                    if updated_brokers_list_1 or updated_brokers_list_2:
                        update_brokers_data(
                            ...
                            updated_brokers_list_1,
                            updated_brokers_list_2,
                        )

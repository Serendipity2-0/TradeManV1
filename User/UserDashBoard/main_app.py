...
from login_page import login_page, session_state
from profile_page import show_profile

strategy_table_names  = ['AmiPy', 'MPWizard', 'OvernightFutures', 'ExpiryTrader','PyStocks', 'ExtraTrades', 'ErrorTrade']
...
def main():
    ...
    if session_state.logged_in:
        ...
        username = session_state.client_data['TradeManID']  # Adjust based on actual data
        ...
        user_strategy_table_names = [table for table in table_names if table in strategy_table_names]

        # Sidebar for strategy selection
        selected_strategy = st.sidebar.radio("Choose a strategy", user_strategy_table_names)
        ...
        data = process_tables(os.path.join(folder_path, selected_file), selected_strategy)
        
        dtd_data = process_tables(os.path.join(folder_path, selected_file), 'DTD')
        
        transactions_data = process_tables(os.path.join(folder_path, selected_file), 'Transactions')
        
        holdings_data = process_tables(os.path.join(folder_path, selected_file), 'Holdings')
            
        account_value = 2500000
        port_stats = PortfolioStats(dtd_data, account_value)

        # Create tabs for 'Data', 'Stats', and 'Charts'
        tab1, tab2, tab3, tab4, tab5, tab6,tab7 = st.tabs(["Profile", "PortfolioView","Strategy Data","Holdings","DailyTrades","Transactions","UserTrades"])

...
def moving_average(df, HA_MA_period):
    ...
    MA_high = pd.Series(
        df["high"].rolling(HA_MA_period, min_periods=HA_MA_period).mean(),
        name="MA_high",
    )
    MA_low = pd.Series(
        df["low"].rolling(HA_MA_period, min_periods=HA_MA_period).mean(), name="MA_low"
    )
    MA_close = pd.Series(
        df["close"].rolling(HA_MA_period, min_periods=HA_MA_period).mean(),
        name="MA_close",
    )
...
def heikin_ashi(df):
    ...
    ha_df["ha_close"] = (
        df["MA_open"] + df["MA_high"] + df["MA_high"] + df["MA_close"]
    ) / 4
    ...
    ha_df["ha_high"] = (
        df["MA_high"].combine(ha_df["ha_close"], max).combine(ha_df["ha_open"], max)
    )

    # HaLow = Min(Low, Min(HaClose, HaOpen))
    ha_df["ha_low"] = (
        df["MA_close"].combine(ha_df["ha_close"], min).combine(ha_df["ha_open"], min)
    )
...
def super_trend(df, period=9, multiplier=7):
    ...
    atr = df["ha_high"].rolling(period).apply(lambda x: np.max(x[:-1] - x[-1])) + df[
        "ha_low"
    ].rolling(period).apply(lambda x: np.min(x[:-1] - x[-1]))
...
def supertrend_new(df, period=9, multiplier=7):
    ...
    result["TrendSL"] = np.where(
        result["Trend"] == 1, result["TrendUp"], result["TrendDown"]
    )
...
def supertrend1(df, factor, pd):
    ...
    for i in range(1, len(result)):
        ...
        if result.at[i, "Trend"] == 1:
            result.at[i, "supertrend"] = min(
                result.at[i, "TrendUp"], result.at[i - 1, "supertrend"]
            )
        else:
            result.at[i, "supertrend"] = max(
                result.at[i, "TrendDown"], result.at[i - 1, "supertrend"]
            )

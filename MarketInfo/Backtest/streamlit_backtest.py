...
def calculate_columns(df, start_capital):
    ...
    for i in range(len(df)):
        ...
        total_charges = (
            brokerage + transaction_charges + gst + sebi_charges + stamp_charges
        )
...
df = df.loc[
    (df["Datetime"] >= pd.Timestamp(selected_dates[0]))
    & (df["Datetime"] <= pd.Timestamp(selected_dates[1]))
]
...
Margin_utilized, Lots, Qty, PnL, Taxes, NetPnL, Capital = calculate_columns(
    df, start_capital
)
...
max_consecutive_wins = (
    (df["PnL"] > 0).astype(int).groupby(df["PnL"].astype(bool).cumsum()).count().max()
)
...
max_percent_drawdown = (
    max_drawdown / pd.Series(Capital).loc[drawdowns.idxmin()]
) * 100  # in percentage

recovery_factor = df["NetPnL"].sum() / max_drawdown
risk_reward_ratio = abs(
    df["PnL"][df["PnL"] > 0].mean() / df["PnL"][df["PnL"] < 0].mean()
)
sharpe_ratio = (
    df["NetPnL"].mean() / df["NetPnL"].std() * np.sqrt(len(df))
...
fig.add_trace(
    go.Scatter(
        x=df["Datetime"], y=df["NetPnL"].cumsum(), mode="lines+markers", name="NetPnL"
    )
)
fig.update_layout(
    title="Net Profit and Loss Over Time",
    xaxis_title="Date",
    yaxis_title="Net PnL",
    autosize=False,
    width=1000,
